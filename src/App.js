...
const cancelBooking = async (id) => {
  ...
  try {
    const res = await axios.delete(`http://localhost:5001/bookings/${id}`);

    // ✅ Try both response formats
    let updatedBooking = res.data.booking || res.data;

    // ✅ Handle case where backend returned 200 OK but no data
    if (!updatedBooking && res.status === 200) {
      console.warn("Backend cancelled booking but returned no data — proceeding with UI update.");
      updatedBooking = { id, status: "Cancelled" };
    } else if (!updatedBooking) {
      console.error("No booking data returned from backend");
      return alert("Cancellation failed — backend did not respond properly.");
    }

    // ✅ Update UI to reflect cancellation (don’t remove the card)
    setBookings((prev) =>
      prev.map((b) =>
        b.id === id ? { ...b, ...updatedBooking, status: "Cancelled" } : b
      )
    );

    console.log(`✅ Booking ${id} marked as Cancelled.`);
  } catch (err) {
    console.error("Error cancelling booking:", err);
    alert("Failed to cancel booking. Check console for details.");
  }
}; // ✅ THIS curly brace and semicolon are REQUIRED — must be here, not missing or duplicated
  ...
  return (
    <div>
      ...
      <form className="booking-form" onSubmit={handleSubmit}>
        ...
        <input
          type="text"
          name="corporateBankName"
          value={formData.corporateBankName}
          onChange={handleChange}
        />
      ...
      <div className="bookings-container">
        <h2 style={{ textAlign: "center", marginTop: "30px" }}>Bookings</h2>
        {bookings.length === 0 ? (
          <p>No bookings yet.</p>
        ) : (
  bookings.map((b, idx) => (
    <div key={idx} className={`booking-card ${b.status === "Cancelled" ? "cancelled" : ""}`}>
...
{b.status !== "Cancelled" ? (
  ...
  <button className="cancel-button cancelled" disabled>
    Cancelled
  </button>
)}
             <div className="staff-section">
                <label>
                  ...
                  <input
                    ...
                    value={b.billAmount || ""}
                    onChange={(e) =>
                      updateBookingField(b.id, "billAmount", e.target.value)
                    }
                  />
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    value={b.serviceBy || ""}
                    onChange={(e) =>
                      updateBookingField(b.id, "serviceBy", e.target.value)
                    }
                  />
                </label>
                ...
                <label>
                  ...
                  <select
                    value={b.status || ""}
                    onChange={(e) =>
                      updateBookingField(b.id, "status", e.target.value)
                    }
                  >
                ...
                <label>
                  ...
                  <select
                    value={b.action || ""}
                    onChange={(e) =>
                      updateBookingField(b.id, "action", e.target.value)
                    }
                  >
